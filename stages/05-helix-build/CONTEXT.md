...
Build Helix from scratch against vault/helix-architecture.md. Implement FSRS and artifact-based state reading.

## Inputs

| Source | File/Location | Section/Scope | Why |
|--------|--------------|---------------|-----|
| Helix design | `../00-d-helix-design/output/` | All five artifacts | Spec to implement |
...
3. Implement artifact-based state reading: Helix reads `progress/progress.json` + mission-command filesystem before routing any request
...
| Artifact state reads | Helix reads progress.json + filesystem before routing; "hi" produces a grounded response |
...
| `helix-invocation-guide.md` | `output/` | How students invoke Helix from the terminal: command syntax, flag options, when to use Helix vs reading the site. Included in Zone 01 lesson as the course's first Helix touchpoint. |
