...
## Cursor rules (always-on in Agent — `.cursor/rules/`)
...
| Who | Role | Use for |
|-----|------|---------|
| **Otto** (Cursor Agent) | Read-only advisor | Plans, review, handbook, counselor, placement |
| **Cline** | Edits, commits, factory | `quiz-factory/`, lesson fixes, scripts |
| **Claude Code** in `quiz-factory/` | Batch quiz generation | Manifest rows only — not tutoring |
...
Edit `.cursor/skills/student-handbook/SKILL.md` when you add skills, rules, or modes. Keep one row per resource; delete hype. Commit when ready.
