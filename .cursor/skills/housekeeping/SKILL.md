...
- Duplicate policy across `.cursor/rules/`, `AGENTS.md`, skills, README
- Multiple `alwaysApply: true` rules saying the same thing
- Skills that should be archived (one-time setup done) → `.cursor/_archive/skills/`
...
| Topic | Single source |
|-------|----------------|
| Local code graph | `.cursor/rules/local-graph.mdc` + `/refresh` skill |
| Agent roles | `.cursor/rules/consultant-charter.mdc` |
| Lesson quizzes (quality) | `.cursor/skills/lesson-planning/SKILL.md` |
| Batch quiz queue | `quiz-factory/` (maintainers) |
| Learner index | `.cursor/skills/student-handbook/SKILL.md` |

### Folder bloat

- Untracked/generated dirs that should be gitignored (not committed): local graph cache dir, `catalog.json`, `output.txt`, etc.
- Duplicate trees (`.agents/` mirroring `.cursor/` if both exist)
...
- **Archive** don't delete when history helps: move to `.cursor/_archive/` with `disable-model-invocation: true` and stripped trigger phrases.
...
**Agent:** Narrow scan to `.cursor/` + `AGENTS.md`, still report before edits.
